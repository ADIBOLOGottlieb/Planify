...
import 'package:planify/widgets/statusBox.dart';
...
Map<String, String> productIDs = {
  'yearly': getPlatform(ignoreEmulation: true) == PlatformOS.isIOS
      ? 'planify.pro.yearly' //iOS
      : 'planify.pro.yearly', //Android
  'monthly': getPlatform(ignoreEmulation: true) == PlatformOS.isIOS
      ? 'planify.pro.monthly' //iOS
      : 'planify.pro.monthly', //Android
  'lifetime': getPlatform(ignoreEmulation: true) == PlatformOS.isIOS
      ? 'planify.pro.life' //iOS
      : 'planify.pro.lifetime', //Android
};
...
class planifyProBanner extends StatelessWidget {
  const planifyProBanner({this.large = false, this.fontColor, super.key});
  final bool large;
...
openManagePurchase() {
  if (appStateSettings["purchaseID"] == productIDs["lifetime"]) {
    ...
    openUrl(
        "https://play.google.com/store/account/subscriptions?sku=planify.pro.monthly&package=com.budget.tracker_app");
  } else if (appStateSettings["purchaseID"] == productIDs["yearly"]) {
    openUrl(
        "https://play.google.com/store/account/subscriptions?sku=planify.pro.yearly&package=com.budget.tracker_app");
  } else {
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 10),
            planifyProBanner(fontColor: getColor(context, "black")),
            TextFont(
...
showHelpRestorePopup(BuildContext context) {
  openPopup(
    ...
    onExtra: () =>
        openUrl("https://planifyapp.web.app/faq.html#restoring-purchases"),
    onExtraLabel: "FAQ".tr(),
  );
}
  ...
  Widget build(BuildContext context) {
    Widget background = Container(
      ...
      child: disableAnimation
          ? Container()
          : Container(), // Removed PlasmaRenderer since sa3_liquid was removed
    );
    if (disableAnimation) {
